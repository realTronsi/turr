...
function iceBullet(arena, bullet, delta, b) {
  ...
  bullet.changed["y"] = true;
	...
	for (let collider of collides) {
      ...
      if (arena.players[collider.gameId].effects.frozen <= 0){
        arena.players[collider.gameId].effects.frozen = bullet.stats.effect;
      }
      ...
      if (arena.enemies[collider.id] != undefined){
        const enemy = arena.enemies[collider.id]
				if (enemy.effects.frozen <= 0){
					enemy.effects.frozen = bullet.stats.effect;
				}
