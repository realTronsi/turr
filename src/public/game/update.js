...
export function Update(gameData, delta){
  ...
  for(let i of Object.keys(gameData.players)){
    ...
    player.x = lerp(player.x, player.middleX, delta*serverTick)
    player.y = lerp(player.y, player.middleY, delta*serverTick)
    player.middleX = lerp(player.middleX, player.serverX, delta*serverTick)
    player.middleY = lerp(player.middleY, player.serverY, delta*serverTick)
  }
  if(true){
