...
const TowerDescriptions = {
  ...
  "base": [`hacker, how can you place`, `bases? teach me plz`]
}
...
const BulletSprites = {
  ...
  air: {
    ...
    red: createImage("../../assets/bullets/air_red.svg")
  },
...
export function Render(gameData, ctx, canvas, held, mouse, canPlace, leaderboard, gameMessages, deathScreenOpacity, respawnTime) {
  ...
  for (let id of Object.keys(gameData.enemies)) {
    ...
    ctx.rotate(enemy.dir)
    ctx.drawImage(EnemySprites[enemy.type], - enemy.size, - enemy.size, enemy.size * 2, enemy.size * 2);
    ctx.rotate(-enemy.dir);
    ctx.translate(-enemy.x, -enemy.y)
    ...
    ctx.lineWidth = 8;
    ctx.beginPath();
    ctx.moveTo(enemy.x - enemy.size, enemy.y + enemy.size + 10);
    ctx.lineTo(enemy.x + enemy.size, enemy.y + enemy.size + 10);
    ctx.stroke();
    ctx.strokeStyle = "rgb(250, 0, 0)"
    ctx.lineWidth = 6;
    ctx.beginPath();
    ctx.moveTo(enemy.x - enemy.size, enemy.y + enemy.size + 10);
    ctx.lineTo(enemy.x - enemy.size + (enemy.size * 2 * enemy.hp / enemy.maxHP), enemy.y + enemy.size + 10);
    ctx.stroke();
  }
  ...
  if (gameData.you.dead != true) {
    ...
    for (let pos of gameData.enemyMinimap) {
      ctx.beginPath();
      ctx.fillStyle = "rgb(255, 0, 0)"
      ctx.globalAlpha = 0.3;
      ctx.arc(20 + 200 * pos.x / gameData.arenaWidth, 680 + 200 * pos.y / gameData.arenaHeight, 4, 0, Math.PI * 2);
